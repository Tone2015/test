	...
	/*$.ajax({
		...
		success:function(data){
			...
			wx.onMenuShareTimeline({
				title: _weixinData.name, // 分享标题
				link: window.location.href.split('#')[0], // 分享链接
				//link: 'http://icoolapp.cn/ShowApp/toShowUserApp?id='+data.appId,
				imgUrl: "http://"+window.location.hostname+_weixinData.img, // 分享图标
				success: function () { 
			...
			wx.onMenuShareAppMessage({
				title: _weixinData.name,
				desc: _weixinData.describe, 
				link: window.location.href.split('#')[0], 
				imgUrl: "http://"+window.location.hostname+_weixinData.img, 
				success: function () { 
...
$(function(){
	...
	if(checkEquipment() == "pc"){
		...
		$("body").css({
			"background-image":"url(http://app.icoolapp.cn/Public/appRes/appbg/"+getRandom(19)+".jpg)" //随机数1~5
		});
...
function jumpToPage(num,_this){
	...
	$.ajax({
		...
		success:function(data)
		{
			if(data.info=="ok")
				{
				$("#tpBtn").attr("src","/Public/Home/Images/img/ytp.png");
				$("#isTp").val("1");
				var thisIndex = $(_this).parents(".page").index()+1;
				var nowPage = ".page-"+num+"-1",
					lastPage = ".page-"+thisIndex+"-1";
